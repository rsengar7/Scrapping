...
class EcomedesSpider(scrapy.Spider):
    name = 'ecomedes'
    allowed_domains = ['ecomedes.com']
    start_urls = ['https://level.ecomedes.com/?level-certificate.kind=1%7C2%7C3']

    def parse(self, response):
        product_urls=response.xpath('//*[@class="col-sm-4"]/div/a/@href').extract()

        for product_url in product_urls:
            abs_product_url='https://level.ecomedes.com'+product_url
        ...
        if int(current_page) <=pages:
                next_page_url='https://level.ecomedes.com/?level-certificate.kind=1%7C2%7C3&page='+str(current_page)
